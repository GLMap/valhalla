...
GraphTileBuilder::GraphTileBuilder(const std::string& tile_dir,
  ...
  // Preserve spatial lookup data when rewriting an existing tile.
  for (size_t i = 0; i < kBinCount; ++i) {
    const auto bin = GetBin(i);
    bins_builder_[i].assign(bin.begin(), bin.end());
    const auto circles = GetBoundingCircles(i);
    bounding_circles_builder_[i].assign(circles.begin(), circles.end());
  }

  // Predicted profiles must move with the other variable-sized sections.
  if (header_->predictedspeeds_count() > 0) {
    ...
    for (uint32_t i = 0; i < speed_profile_builder_.size(); i += kCoefficientCount) {
      speed_profile_index_.emplace(i);
    }
  }
...
void GraphTileBuilder::StoreTileData() {
  ...
  if (header_builder_.has_bounding_circles()) {
    header_builder_.set_bounding_circle_offset(sizeof(GraphTileHeader) + in_mem.tellp());
    for (const auto& circles : bounding_circles_builder_) {
      in_mem.write(reinterpret_cast<const char*>(circles.data()),
                   circles.size() * sizeof(DiscretizedBoundingCircle));
    }
  }

  // Write the forward complex restriction data.
  header_builder_.set_complex_restriction_forward_offset(sizeof(GraphTileHeader) + in_mem.tellp());
  ...
  // Preserve predicted speeds after names/elevations have changed their position.
  ...
  if (!speed_profile_builder_.empty()) {
    ...
    header_builder_.set_predictedspeeds_offset(sizeof(GraphTileHeader) + in_mem.tellp());
  ...
  header_builder_.set_end_offset(sizeof(GraphTileHeader) + in_mem.tellp());
...
void GraphTileBuilder::Update(const std::vector<NodeInfo>& nodes,
  ...
  // Extension records are indexed by directed edge and must stay in place.
  if (header_->has_ext_directededge()) {
    in_mem.write(reinterpret_cast<const char*>(ext_directededges_),
...
valhalla::baldr::Sign& GraphTileBuilder::sign(const size_t idx) {
  if (idx < header_->signcount()) {
    return const_cast<Sign&>(signs_[idx]);
  }

...
void CheckLuaFuncExists(lua_State* state, const std::string& func_name) {
  ...
  if (!lua_isfunction(state, -1)) {
    throw std::runtime_error(fmt::format("Lua script does not contain a function {}.", func_name));
  }
  lua_pop(state, 1);
}
  ...
  try {
    ...
    if (lua_pcall(state_, 2, type == OSMType::kWay ? 4 : 2, 0)) {
      ...
      throw std::runtime_error(fmt::format(
          "Failed to execute lua function for basic tag processing in {} {}: {}",
    ...
    while (lua_next(state_, -2) != 0) {
      ...
      if (key == nullptr) {
        LOG_ERROR(fmt::format("Invalid key in Lua function: {}.", lua_func));
        break;
      }
      const char* value = lua_tostring(state_, -1);
      if (value == nullptr) {
        LOG_ERROR(fmt::format("Invalid value in Lua function: {}.", lua_func));
        break;
      }
      result[key] = value;
      lua_pop(state_, 1);
    ...
    LOG_ERROR(fmt::format("Exception in Lua function: {}: {}", lua_func, e.what()));
  } catch (...) {
    LOG_ERROR(fmt::format("Unknown exception in Lua function: {}.", lua_func));
  }

    ...
    if (edge.use() == Use::kTransitConnection || edge.use() == Use::kEgressConnection ||
      ...
      if (edge.is_shortcut()) {
        LOG_ERROR(fmt::format(
            "No opposing shortcut edge at LL={},{} Length = {} Startnode {} EndNode {}", ll.lat(),
            ll.lng(), edge.length(), fmt::streamed(startnode), fmt::streamed(edge.endnode())));
      } else {
        LOG_ERROR(fmt::format(
            "No opposing edge at LL={},{} Length = {} Startnode {} EndNode {} WayID {} "
            "EdgeInfoOffset {}",
            ll.lat(), ll.lng(), edge.length(), fmt::streamed(startnode),
            fmt::streamed(edge.endnode()), wayid, edge.edgeinfo_offset()));
      }
#endif
      ...
      for (uint32_t i = 0; i < nodeinfo->edge_count(); i++, directededge++) {
        if (edge.is_shortcut() == directededge->is_shortcut()) {
          LOG_WARN(fmt::format("    Length = {} Endnode: {} WayId = {} EdgeInfoOffset = {}",
                               directededge->length(), fmt::streamed(directededge->endnode()),
                               end_tile->edgeinfo(directededge).wayid(),
...
void GraphValidator::Validate(const boost::property_tree::ptree& pt) {
  ...
  for (uint8_t level = 0; level < TileHierarchy::levels().size(); level++) {
    ...
    LOG_WARN(fmt::format("Possible duplicates at level: {} = {}", level, duplicates[level]));

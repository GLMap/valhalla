...
static std::vector<std::string> GetTokens(const std::string& tag_value, char delim) {
  std::vector<std::string> tokens;
  boost::algorithm::split(tokens, tag_value, [=](auto ch){return ch==delim;},
                          boost::algorithm::token_compress_on);
  return tokens;
}

static bool RegexFound(const std::string& source, const re::regex& regex) {
  auto begin = re::sregex_iterator(source.begin(), source.end(), regex);
  auto end = re::sregex_iterator();
  return std::distance(begin, end);
}

static std::string FormatCondition(const std::string& source, const re::regex& regex, const std::string& pattern) {

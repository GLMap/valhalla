...
graph_tile_ptr GraphTile::Create(const GraphId& graphid, const std::string& file, uint32_t offset, uint32_t size) {
  ...
  auto mmap = MemoryMapHandle(fd, offset, size);
  ::close(fd);
  
  if(!mmap)
    return nullptr;
  
  return new GraphTile(graphid, std::make_unique<const MMapGraphMemory>(std::move(mmap), size));
}
...
void GraphTile::Initialize(const GraphId& graphid) {
  ...
  bounding_circles_ =
      reinterpret_cast<const DiscretizedBoundingCircle*>(tile_ptr + header_->bounding_circle_offset());
...
GraphTile::GetAccessRestrictions(const uint32_t idx) const {
  ...
  return {std::span<const AccessRestriction>(access_restrictions_ + start, access_restrictions_ + found),
          start};
}
...
std::span<const DiscretizedBoundingCircle> GraphTile::GetBoundingCircles(size_t column, size_t row) const {
  if (!header_->has_bounding_circles()) {
    return std::span<const DiscretizedBoundingCircle>{bounding_circles_, bounding_circles_};
  }
  auto offsets = header_->bin_offset(column, row);
  return std::span<const DiscretizedBoundingCircle>{bounding_circles_ + offsets.first,
                                              bounding_circles_ + offsets.second};
}

std::span<const DiscretizedBoundingCircle> GraphTile::GetBoundingCircles(size_t index) const {
  if (!header_->has_bounding_circles()) {
    return std::span<const DiscretizedBoundingCircle>{bounding_circles_, bounding_circles_};
  }
  auto offsets = header_->bin_offset(index);

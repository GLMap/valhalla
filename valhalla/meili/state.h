...
class StateContainer
{
 ...
 public:
  ...
  void AppendState(const State& state)
  {
    ...
    const auto expected_time = static_cast<StateId::Time>(columns_.size() - 1);
    const auto expected_id = static_cast<StateId::Id>(columns_.back().size());
    if (state.stateid() != StateId(expected_time, expected_id)) {
      throw std::runtime_error( "state's stateid should be " + std::to_string(expected_time) + "/" +
        std::to_string(expected_id) + " but got " + std::to_string(state.stateid().time()) + "/" +

...
TEST(GraphTileBuilder, PreserveSpatialIndexAndTrafficWhenRewriting) {
  ...
  const std::string source_dir = "test/data/rewrite_source";
  {
    ...
    fixture.directededges_ext().resize(2);
    fixture.directededges_ext()[0].set_hiking_seconds(123);
    fixture.directededges_ext()[1].set_hiking_seconds(456);
    bool added = false;
  ...
  for (const bool predicted : {false, true}) {
    ...
    bins_t bins;
    bins.front() = {{GraphId(818660, 2, 0), DiscretizedBoundingCircle()},
                    {GraphId(818661, 2, 7), DiscretizedBoundingCircle()}};
    bins.back() = {{GraphId(818660, 2, 1), DiscretizedBoundingCircle()}};
    GraphTileBuilder::AddBins(tile_dir, source, bins, predicted);
    {
      ...
      std::vector<NodeInfo> nodes(traffic.GetNodes().begin(), traffic.GetNodes().end());
      traffic.Update(nodes, edges);
      GraphTileBuilder after_update(tile_dir, id, false);
      ASSERT_TRUE(after_update.header()->has_ext_directededge());
      EXPECT_EQ(after_update.ext_directededge(0)->hiking_seconds(), 123);
      EXPECT_EQ(after_update.ext_directededge(1)->hiking_seconds(), 456);
    ...
    for (size_t pass = 0; pass < 2; ++pass) {
      ...
      EXPECT_NE(after->header()->tile_checksum(), before->header()->tile_checksum());
      ASSERT_TRUE(after->header()->has_ext_directededge());
      EXPECT_EQ(after->ext_directededge(0)->hiking_seconds(), 123);
      EXPECT_EQ(after->ext_directededge(1)->hiking_seconds(), 456);
      EXPECT_EQ(after->header()->has_bounding_circles(), predicted);
      if (predicted) {
      ...
      for (size_t i = 0; i < kBinCount; ++i) {
        ...
        const auto a_circles = before->GetBoundingCircles(i);
        const auto b_circles = after->GetBoundingCircles(i);
        ASSERT_EQ(a_circles.size(), b_circles.size());
        EXPECT_TRUE(std::equal(a_circles.begin(), a_circles.end(), b_circles.begin()));
      }
